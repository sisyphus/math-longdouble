...
=head1 OVERLOADING
   ...
   The following operations are overloaded:
    ...
    See the powq_LD documentation (below) for the option that allows
    powq() to be used instead of powl() in the overloading of '**'
    and '**='.
...
=head1 MATH LIBRARY FUNCTIONS
   ...
   pow_LD($rop, $op1, $op2);
    pow($op1, $op2) is assigned to $rop. (Uses C powl() function.)

   powq_LD($rop, $op1, $op2);
    As for pow_LD(), except that $op1 and $op2 are cast to __float128
    values, and the calculation is done using the quadmath library's
    powq() function - the result of which is cast back to a long
    double and returned to $rop.
    The aim here is to provide a precise alternative as powl() is
    generally buggy in the underlying math library implementation.
    This function will be made available automatically only if the
    'perl Makefile.PL' processing detects that powq() is available.

    If powq_LD() is provided then powq() will also be used in the
    same way in the '**' and '**=' overloading if (and only if) the
    command line argument ALLOW_POWQ_OLOAD was provided at the
    'perl Makefile.PL' stage, eg 'perl Makefile.PL ALLOW_POWQ_OLOAD'.

    NOTE: It's highly likely that use of powq_LD produces a correct
          result - but that's not guaranteed, owing to the double
