...
=head1 MATH LIBRARY FUNCTIONS
   ...
   powq_LD($rop, $op1, $op2);
    ...
    The aim here is to provide a precise alternative when powl() is
    buggy. This function will be made available automatically only if
    it is detected that powl() is buggy && that powq() is available.
    The function can be made explicitly unavailable by providing the
    'USE_POWL' argument to the 'perl Makefile.PL' command.
    If powq_LD() is provided then powq() will also be used in the
    sameway in the '**' and '**=' overloading. (Otherwise powl() will
    be used in that overloading.)
